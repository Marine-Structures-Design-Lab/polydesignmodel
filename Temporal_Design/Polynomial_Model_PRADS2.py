...
analysis = [[sp.Eq(x[0] + x[1] - x[2])],                                 # A1
            [sp.Eq(x[2] - x[3]**2 - x[4]), sp.Eq(2*x[1] + x[3] - x[5])], # A2
            [sp.Eq(x[4] + sp.sqrt(x[5]) - x[6] - x[7])],                 # A3
            [sp.Eq(x[7] - 2*(x[8]+x[0]**3) - x[9])]];                    # A4
...
for i in range(0,np.shape(Path_vals)[0]):        # i loops with paths
    for j in range(0,runs):                      # j loops with runs
        ...
        # Retrieve needed inputs for first analysis
        index = sequence[i][0]
        variables = getVariables(analysis[index-1])
        Vars = variables.getVars()
        
        # Get random inputs for first analysis
